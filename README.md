...
├── OS: Ubuntu / Linux Power User
├── Venture: Founder of [1337 Wing](https://agentred1999.github.io/1337-Wing)
└── Philosophy: Build it $\rightarrow$ Break it $\rightarrow$ Fix it $\rightarrow$ Ship it.

$ pwd
~/houston-tx/dev-workflow
```
...
```bash
$ ./tech_stack.sh --list
Frontend    $\rightarrow$ Next.js, TypeScript, React, Redux, Tailwind
Backend     $\rightarrow$ Node.js, Firebase, Python, PostgreSQL
Infrastructure $\rightarrow$ Ubuntu, Bash, Docker, Git, Meshtastic
```
...
📂 skin-ai       $\rightarrow$ AI skincare analysis (Next.js 15 / TS)
📂 summarist     $\rightarrow$ Blinkist-style SaaS (Firebase / Stripe)
📂 1337-wing     $\rightarrow$ Cyber hardware brand (React / Vite)
📂 nft-market    $\rightarrow$ Dynamic NFT Marketplace (Firebase / React)
```
...
```bash
$ exit
␛[0m Thanks for stopping by. See you in the terminal... █ ␛[0m 
``` 
